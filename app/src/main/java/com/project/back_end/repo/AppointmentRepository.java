...
public interface AppointmentRepository extends JpaRepository<Appointment, Long> {
    ...
    // Retrieve appointments for a doctor within a time range (fetch doctor & patient)
    ...
    // Filter appointments by doctor ID, patient name (partial, case-insensitive), and time range
    @Query("SELECT a FROM Appointment a " +
           "LEFT JOIN FETCH a.doctor d " +
           "LEFT JOIN FETCH a.patient p " +
           "WHERE d.id = :doctorId " +
    ...
    // Find all appointments for a patient by status, ordered by appointment time
    ...
    // Search appointments by partial doctor name, patient ID, and status
    @Query("SELECT a FROM Appointment a " +
           "LEFT JOIN a.doctor d " +
           "WHERE LOWER(d.name) LIKE LOWER(CONCAT('%', :doctorName, '%')) " +
           "AND a.patient.id = :patientId " +
