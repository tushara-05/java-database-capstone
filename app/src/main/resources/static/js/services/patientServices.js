...
export async function filterAppointments(condition, name, token) {
  try {
    ...
    const response = await fetch(`${PATIENT_API}/filter/${searchCondition}/${searchName}/${token}`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    // Return filtered data if successful
    if (response.ok) {
      const data = await response.json();
      return { ...data, success: true };
    } else {
      // Log error and return empty list with failure status
      console.error("Failed to fetch appointments:", response.statusText);
      return { appointments: [], success: false, message: "Failed to filter appointments" };
    }
  } catch (error) {
    // Handle unexpected errors and return consistent format
