  ...
  if (appointmentId && token) {
    try {
      ...
      if (response && response.prescription) {
        ...
        // If a prescription exists AND we're NOT in view mode, it's an update
        if (mode !== "view") {
          if (heading) heading.innerHTML = `Update <span>Prescription</span>`;
          if (savePrescriptionBtn) savePrescriptionBtn.textContent = "Update Prescription";
        }
      } else if (mode === "view") {
