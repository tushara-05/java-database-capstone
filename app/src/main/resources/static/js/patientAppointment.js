...
function renderAppointments(appointments) {
  ...
  appointments.forEach(appointment => {
    ...
    tr.innerHTML = `
      ...
      <td>${appointment.status == 0 ? `<img src="../assets/images/edit/edit.png" alt="Edit Appointment" title="Edit Appointment" class="action-btn edit-btn" data-id="${appointment.id}">` :
        appointment.status == 1 ? `<img src="../assets/images/addPrescriptionIcon/addPrescription.png" alt="View Prescription" title="View Prescription" class="action-btn view-btn" data-id="${appointment.id}">` :
          "Cancelled"
      }</td>
    `;
...
async function handleFilterChange() {
  ...
  try {
    if (!patientId) return;
    const response = await filterAppointments(patientId, condition, name, token);
