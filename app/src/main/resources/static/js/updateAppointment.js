...
async function initializePage() {
  ...
  getDoctors()
    .then(doctors => {
      ...
      // Handle date change to filter times
      const now = new Date();
      const todayLocal = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}`;

      const updateTimes = () => {
        const selectedDate = appointmentDateEl.value;
        let timesToShow = availableTimes;

        if (selectedDate === todayLocal) {
          const currentHours = now.getHours();
          const currentMinutes = now.getMinutes();
          timesToShow = availableTimes.filter(t => {
            const startTimeStr = t.includes('-') ? t.split('-')[0].trim() : t.trim();
            const [h, m] = startTimeStr.split(':').map(Number);
            return (h > currentHours) || (h === currentHours && m > currentMinutes);
          });
        }

        const previousValue = appointmentTimeEl.value;
        appointmentTimeEl.innerHTML = '<option value="">Select time</option>';
        ...
        // Restore previous value if available in new list
        if (timesToShow.includes(previousValue)) {
            appointmentTimeEl.value = previousValue;
        }
      };
      ...
      // If the URL has a time, try to pre-select it after population
      if (appointmentTimeEl) appointmentTimeEl.value = appointmentTime;

      // Add listener for date changes
      appointmentDateEl.addEventListener("change", updateTimes);
